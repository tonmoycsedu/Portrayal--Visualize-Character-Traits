class Discourse extends Card {
    constructor(feat_name, data, data_key, selected_characters, div_id, height, description) {
        ...
        self.chart = self.visualize_discourse()
        $("#" + self.svg.svg_id).append(self.chart)
        self.chart.update(self.selected_data, self.x_domain)
        $("#" + self.feat_name + "_meta").append('<input id= "normalize_discourse" type = "checkbox" > Normalize by chapter length')
    ...
    data_prep(data, feat_name, selected_part = false, selected_chapter = -1, selected_characters = []) {
        ...
        if (selected_chapter < 0) {
            selected_data = d3.flatRollup(data, v => d3.sum(v, d => d[feat_name]),
                ...
                self.svg.bars = self.svg.bars
                    ...
                    .join(
                        enter => enter.append("rect")
                            .attr("class", "direct_discourse_bar features_unit")
    ...
    install_local_events() {
        var self = this;
        $("#normalize_discourse").on("change", function () {
